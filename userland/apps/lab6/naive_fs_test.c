...
static int test_naive_fs_creat()
{
    ...
    for (int i = 0; i < 10; ++i) {
        if (naive_fs_access(pn_buf) != -1)
            return -1;
        // printf("pass create stage 1\n");    
        if (naive_fs_creat(pn_buf) != 0)
            return -1;
        // printf("pass create stage 2\n");    
        if (naive_fs_access(pn_buf) != 0)
            return -1;
        // printf("pass create stage 3\n");    
        pn_buf[8]++;
    }
