...
void *producer(void *arg)
{
        ...
        while (i < PROD_ITEM_CNT) {
                ...
                // printf("empty_slot in producer = %d\n", empty_slot);
                /* LAB 4 TODO END */
                new_msg = produce_new();
                // printf("finish produce_new function!\n");
                buffer_add_safe(new_msg);
                // printf("finish buffer_add_safe function!\n");
                /* LAB 4 TODO BEGIN */
                __chcore_sys_signal_sem(filled_slot);
                // printf("filled_slot in producer = %d\n", filled_slot);
                /* LAB 4 TODO END */
                i++;
        }
...
void *consumer(void *arg)
{
        ...
        while (i < COSM_ITEM_CNT) {
                ...
                // printf("filled_slot in consumer = %d\n", filled_slot);
                /* LAB 4 TODO END */
                cur_msg = buffer_remove_safe();

                /* LAB 4 TODO BEGIN */
                __chcore_sys_signal_sem(empty_slot);
                // sprintf("empty_slot in consumer = %d\n", empty_slot);
                /* LAB 4 TODO END */
                consume_msg(cur_msg);
                i++;
        }
        // printf("ready to add 1 in consumer func!\n");
        __sync_fetch_and_add(&global_exit, 1);
        return 0;
}
