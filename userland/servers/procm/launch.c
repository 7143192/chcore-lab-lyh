...
int launch_process(struct launch_process_args *lp_args)
{
        ...
        int *child_process_cap = lp_args->child_process_cap; // current process(main thread) cap.
        int *child_main_thread_cap = lp_args->child_main_thread_cap; // child thread cap. 
        int *caps = lp_args->caps;
        int nr_caps = lp_args->nr_caps;
        s32 cpuid = lp_args->cpuid;// affinity
        int argc = lp_args->argc;
        char **argv = lp_args->argv;
        int pid = lp_args->pid;// process id.
        u64 pcid = lp_args->pcid;// process context id.
        ...
        construct_init_env(init_env,
                           ...
                           pid);
        free(transfer_caps);
        // write the init env to the stack, and the init env size is 0x1000. 
        ...
        // printf("[DEBUG] finish create new main thread in launch.c!\n");
