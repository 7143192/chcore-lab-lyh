...
static int lab5_stdio_file_read_write () {
    ...
    FILE * pFile;
	printf("ready to open file /myfile.txt!\n");
    pFile = fopen("/myfile.txt", "w");
	printf("open /myfile.txt finished!\n");
    fwrite(wbuf, sizeof(char) , sizeof(wbuf), pFile);
	printf("write /myfile.txt finished!\n");
    fclose(pFile);
	printf("close /myfile.txt finished!\n");

    pFile = fopen("/myfile.txt", "r");
	printf("reopen /myfile.txt file!\n");
    int cnt;
    do {
        cnt = fread(rbuf, sizeof(char), sizeof(rbuf), pFile);
    } while(cnt > 0);
	printf("read /myfile.txt finished!\n");
    fclose(pFile);
