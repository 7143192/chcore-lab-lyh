...
static inline struct inode *new_inode(void)
{
	...
	inode->type = 0; // FS_REG = 1, FS_DIR = 2
	inode->size = 0;
			       ...
			       size_t len)
{
	...
	if (!dent)
		return ERR_PTR(-ENOMEM);
	// len here should be the len of the file name!
...
{
	return tfs_mknod(dir, name, len, 0 /* mkdir */ );// create a new regular file here.
}
...
int tfs_namex(struct inode **dirat, const char **name, int mkdir_p)
{
	...
	while(true) {
		...
		memcpy(buff, *name, i);
		buff[i] = '\0';
		// buff is used to record current level file name.
		...
		// set *dirat at the last to satisfy the last requirement of this function.(name ends with '/').
		*dirat = dent->inode;
		*name = curName + 1;
		i = 0;
	}
...
// write memory into `inode` at `offset` from `buf(data)` for length is `size`
		       ...
		       size_t size)
{
	...
	while (size > 0) {
		...
		// the new file size exceeds the old one.
		if (page == NULL) {
...
int tfs_load_image(const char *start)
{
	...
	for (f = g_files.head.next; f; f = f->next) {
		/* LAB 5 TODO BEGIN */
		// we need to store name here as the tfs_namex func can change the original name here.
		curName = f->name;
		// fr every file, search from the root directory???
		dirat = tmpfs_root_dent->inode;
		// find the parent dir of the target file path.
