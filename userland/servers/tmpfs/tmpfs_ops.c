...
int fs_creat(const char *path)
{
	...
	BUG_ON(!path);
	// NOTE: this means the path must start with '/', not "path == "/""!!!
	BUG_ON(*path != '/');
	...
	if (*leaf != '\0') {
		// corresponding file does not create.
...
int tmpfs_mkdir(const char *path, mode_t mode)
{
	...
	BUG_ON(!path);
	BUG_ON(*path != '/');
	// printf("target path to mkdir is = %s\n", path);

	/* LAB 5 TODO BEGIN */
	dirat = tmpfs_root;
	err = tfs_namex(&dirat, &leaf, 1);
	// printf("finish tfs_namex func in tmpfs_mkdir function!\n");
	if (err == 0) {
		// printf("ready to call tfs_mkdir function!\n");
		err = tfs_mkdir(dirat, leaf, strlen(leaf));
	}
	// printf("finish tmpfs_mkdir function!\n");
	/* LAB 5 TODO END */
	return err;
}
