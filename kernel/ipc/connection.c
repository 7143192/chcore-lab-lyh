...
static struct thread *create_server_thread(struct thread *src)
{
        ...
        BUG_ON(new_config == 0);
        new_config->callback = src_config->callback; // this "callback" is the callback function noted in the graph of lab4 doc.
...
u64 sys_ipc_call(u32 conn_cap, struct ipc_msg *ipc_msg, u64 cap_num)
{
        ...
        // // arg is a arg of callback function, so this arg should be the pos of one data in the shared mem???
