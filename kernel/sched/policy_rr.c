 ...
 * Idle thread will **NOT** be in the RQ(Ready Queue).
 */
 ...
 */
// the budget should be the "time slice" the rr policy allocated to every thread every time this thread is scheduled.
...
{
        /* LAB 4 TODO BEGIN */
        // if (current_thread && current_thread->thread_ctx->thread_exit_state == TE_EXITING) {
        //         current_thread->thread_ctx->state = TS_EXIT;
        //         current_thread->thread_ctx->thread_exit_state = TE_EXITED;
        // }
        // if (current_thread && current_thread->thread_ctx->state == TS_RUNNING) {
        //         // as this thread is not exit yet, make it into INTER state.
        ...
        struct thread *cur = current_thread;
        if (cur != NULL && cur->thread_ctx->thread_exit_state == TE_EXITING) {
                cur->thread_ctx->state = TS_EXIT;
                cur->thread_ctx->thread_exit_state = TE_EXITED;
        }
        if (cur != NULL) {
                // this case can only appeat in the policy of preemptive.
                if (cur->thread_ctx->state == TS_RUNNING) {
                        if (cur->thread_ctx->sc->budget) {
                                return 0;
                        }
                        rr_sched_refill_budget(cur, DEFAULT_BUDGET);
                        cur->thread_ctx->state = TS_INTER;
                        rr_sched_enqueue(cur);
                }
        }
        struct thread *thread = rr_sched_choose_thread();
