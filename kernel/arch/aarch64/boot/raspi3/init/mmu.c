...
#define KERNEL_OFF      (0xFFFFFF0000000000)
...
/* shift the number 1 or 0 to set different attr of one PTE. */
#define UXN            (0x1UL << 54)
...
void init_boot_pt(void)
{
        ...
        for (vaddr = PHYSMEM_START; vaddr < PERIPHERAL_BASE; vaddr += SIZE_2M) {
                u64 vaddr1 = vaddr + KERNEL_VADDR;
                boot_ttbr1_l2[GET_L2_INDEX(vaddr1)] = 
                                (vaddr)
                                | UXN
                                | ACCESSED
                                | NG
