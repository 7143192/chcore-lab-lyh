                ...
                vaddr_t start_addr, u64 page_num)
{
        ...
        for (order = 0; order < BUDDY_MAX_ORDER; ++order) {
                // nr_free is used to record the free items number in one free_list.
...
static struct page *merge_page(struct phys_mem_pool *pool, struct page *page)
{
        ...
        //         // the conditions to stop merge:
        //         // if there is no buddy on the cur order or the buddy is allocated, sto pthe merge process.
        //         if (buddy == NULL || buddy->allocated == 1) break;
        //         if (buddy->order != page->order) break;
        //         // remove the buddy page from its old free_list.
        //         list_del(&(buddy->node));
        //         (pool->free_lists[buddy->order]).nr_free--;
        //         // the newly merged page pointer should point to the smaller addr of the current page and its buddy.
...
void buddy_free_pages(struct phys_mem_pool *pool, struct page *page)
{
        ...
        // the newly-merged page will not be allocated here for now.
