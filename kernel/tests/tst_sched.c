...
void tst_sched_param()
{
        ...
        //kinfo("[DEBUG] ready to test for sched part!\n");
        ...
        //kinfo("[DEBUG] finish test for sched enqueue!\n");
        ...
        //kinfo("[DEBUG] finish test for sched dequeue!\n");
        ...
        //kinfo("[DEBUG] finish test choose_sched !\n");
        ...
        { // test sched
                ...
                BUG_ON(sched_enqueue(threads[0]));
                BUG_ON(sched_enqueue(threads[1]));
                //kinfo("[DEBUG] finish sched test part1!\n");

                sched();
                current_thread->thread_ctx->sc->budget = 0;
                BUG_ON(threads[3]->thread_ctx->state != TS_RUNNING);
                BUG_ON(!sched_dequeue(threads[3]));
                //kinfo("[DEBUG] finish sched test part2!\n");

                thread = rr_sched_choose_thread();
                BUG_ON(thread != threads[2]);
                //kinfo("[DEBUG] finish sched test part3!\n");

                sched();
                current_thread->thread_ctx->sc->budget = 0;
                BUG_ON(threads[0]->thread_ctx->state != TS_RUNNING);
                BUG_ON(!sched_dequeue(threads[0]));
                //kinfo("[DEBUG] finish sched test part4!\n");

                thread = rr_sched_choose_thread();
                BUG_ON(thread != threads[1]);
                //kinfo("[DEBUG] finish sched test part5!\n");

                sched();
                current_thread->thread_ctx->sc->budget = 0;
                BUG_ON(threads[3]->thread_ctx->state != TS_RUNNING);
                BUG_ON(!sched_dequeue(threads[3]));
                //kinfo("[DEBUG] finish sched test part6!\n");

                thread = rr_sched_choose_thread();
                BUG_ON(thread != threads[0]);
                //kinfo("[DEBUG] finish sched test part7!\n");

                BUG_ON(current_thread != threads[3]);
                current_thread = NULL;

                BUG_ON(!list_empty(&(rr_ready_queue_meta[cpuid].queue_head)));
                //kinfo("[DEBUG] finish sched test part8!\n");
        }
        kinfo("[DEBUG] finish test for sched!\n");
...
{
        //kinfo("[DEBUG] get into tst_sched_cooperative function!\n");
        tst_sched_param();
        tst_sched_queue();
        if (smp_get_cpu_id() == 0)
...
void tst_sched_aff_param()
{
        ...
        BUG_ON(thread->thread_ctx->type != TYPE_IDLE);
        kinfo("[DEBUG] finish tst_sched_aff_param part 1!\n");

        thread = create_test_thread(0, 6);
        BUG_ON(!sched_enqueue(thread));
        BUG_ON(!sched_dequeue(thread));
        kinfo("[DEBUG] finish tst_sched_aff_param part 2!\n");
        idle_thread = rr_sched_choose_thread();
        BUG_ON(idle_thread->thread_ctx->type != TYPE_IDLE);
        kinfo("[DEBUG] finish tst_sched_aff_param part 3!\n");

        free_test_thread(thread);
        BUG_ON(!list_empty(&(rr_ready_queue_meta[cpuid].queue_head)));
        kinfo("[DEBUG] finish tst_sched_aff_param part 4!\n");
        global_barrier();
}
...
void tst_sched_aff()
{
        ...
        thread = atomic_sched_choose_thread();
        BUG_ON(thread->thread_ctx->type != TYPE_IDLE);
        kinfo("[DEBUG] finish tst_sched_aff part 1!\n");
        global_barrier();
        if (smp_get_cpu_id() == 0) {
