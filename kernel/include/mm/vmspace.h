...
// a vmregion represents a detailed part of the vmspace.
struct vmregion {
        // node <--> a node in vmr_list of vmspace.
        /*in ChCore, the list_head is always used to represents bi-direction list structure.*/
        struct list_head node; /* vmr_list */
        vaddr_t start;
        size_t size;
        vmr_prop_t perm;
        // the physical memory part mapped to this VM part.
        struct pmobject *pmo;
};

// this structure represents a VM space allocated to a process.
struct vmspace {
...
struct pmobject {
        ...
        paddr_t start;
        size_t size;
        // whether this map is done immediately.
        pmo_type_t type;
};
